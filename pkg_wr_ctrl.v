 ...
 * @Module Name : PKG_WR_CTRL
 * @version     : V1.0
 * @Create Time : Do not edit
 * @LastEditTime: 2024-08-03 16:56:10
 */
...
module PKG_WR_CTRL (
    ...
    //output to ram 
    output reg        hram_wen  ,
    output reg [7:0]  hram_waddr,
    output reg [10:0] hram_wdata,

    output reg        lram_wen,
    output reg [7:0]  lram_waddr,
    output reg [10:0] lram_wdata,

    //To judge whether RAM is empty 
    output reg [7:0]  high_real_waddr,
    output reg [7:0]  low_real_waddr,
...
always @(posedge clk or negedge rst_n) begin
    if(rst_n == 1'b0) begin
        high_real_waddr <= 8'b0;
        low_real_waddr <= 8'b0;
    end else if(curr_state==ST_END) begin
        high_real_waddr <= hram_waddr;
        low_real_waddr <= lram_waddr;
    end
end
